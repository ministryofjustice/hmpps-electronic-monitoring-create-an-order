...
context('Scenarios', () => {
  ...
  context('Alcohol Abstinence and Monitoring Requirement - AAMR (Post Release)', () => {
    ...
    it('Should successfully submit the order to the FMS API', () => {
      ...
      const createDeviceWearerRequestExpectation = {
        title: '',
      ...
      cy.writeFile(
        `./test_results/cypress/requests/CEMO003-createDeviceWearer.json`,
        JSON.stringify(createDeviceWearerRequestExpectation, null, 2),
        'utf8',
      )

      cy.task('verifyFMSCreateDeviceWearerRequestReceived', {
        httpStatus: 200,
        body: createDeviceWearerRequestExpectation,
      }).should('be.true')

      cy.wrap(orderId).then(() => {
        const createMonitoringOrderRequestExpectation = {
          case_id: fmsCaseId,
          allday_lockdown: '',
          atv_allowance: '',
        ...
        cy.task('verifyFMSCreateMonitoringOrderRequestReceived', {
          httpStatus: 200,
          body: createMonitoringOrderRequestExpectation,
        }).should('be.true')

        return cy.writeFile(
          `./test_results/cypress/requests/CEMO003-createMonitoringOrder.json`,
          JSON.stringify(createMonitoringOrderRequestExpectation, null, 2),
          'utf8',
        )
      })
