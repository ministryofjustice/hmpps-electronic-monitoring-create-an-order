...
context('orderType', () => {
  ...
  it('Should display content', () => {
    ...
    cy.get('ul')
      ...
      .should('exist')
    cy.get('ul').children().get('li').contains('They are being re-released following the recall.').should('exist')
    cy.get('ul')
      .children()
      .get('li')
      .contains('They meet HDC policy, eligibility criteria, and pass the risk assessment.')
      .should('exist')
    cy.get('p').contains('If you are unsure whether someone is eligible, contact the COM.').should('exist')
