...
const createDeviceWearer = (override: Partial<DeviceWearer> = {}) => {
  return {
    nomisId: 'nomis',
    pncId: 'pnc',
    deliusId: 'delius',
...
context('Installation location page', () => {
  beforeEach(() => {
    cy.task('reset')
  ...
  context('when no fixed address', () => {
    ...
    it('Should grey out fixed address option', () => {
      const page = Page.visit(InstallationLocationPage, {
        orderId: mockOrderId,
      })
  ...
  context('when has fixed address', () => {
    it('Should show fixed address option', () => {
      stubGetOrder({
        ...mockDefaultOrder,
        addresses: [
          {
  ...
  context('when notifying org is home office', () => {
    it('Should show immigration option', () => {
      cy.task('stubSignIn', { name: 'john smith', roles: ['ROLE_EM_CEMO__CREATE_ORDER'] })
