...
const createViewModel = (order: Order, content: I18n, uri: string = '') => {
  ...
  const answers = [
    createAnswer(questions.offence.text, lookup(content.reference.offences, order.installationAndRisk?.offence), uri),
    ...
    createAnswer(questions.riskDetails.text, order.installationAndRisk?.riskDetails, uri),
    createAnswer(
