...
export default class HavePhotoController {
  constructor(
  ...
  update: RequestHandler = async (req: Request, res: Response) => {
    ...
    formData.fileType = AttachmentType.PHOTO_ID
    ...
    if (isValidationResult(result)) {
      ...
      res.redirect(paths.ATTACHMENT.HAVE_PHOTO.replace(':orderId', order.id))
      ...
      order.orderParameters.havePhoto = formData.fileRequired === 'yes'
      const nextPage = this.taskListService.getNextPage(PAGES.havePhoto, order)
