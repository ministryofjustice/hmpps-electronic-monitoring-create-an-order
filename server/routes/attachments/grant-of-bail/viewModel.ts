...
const createFromEntity = (order: Order, res: Response): FileRequiredViewModel => {
  return {
    title: res.locals.content?.pages.haveGrantOfBail.title,
    question: {
      text: res.locals.content?.pages.haveGrantOfBail.questions.haveGrantOfBail.text,
    },
    fileRequired: {
      value: convertHavePhotoBoolToString(order.orderParameters?.haveGrantOfBail),
    },
    errorSummary: null,
  }
}
