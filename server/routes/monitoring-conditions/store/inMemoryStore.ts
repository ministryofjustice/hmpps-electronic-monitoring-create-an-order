...
export default class InMemoryMonitoringConditionsStore implements MonitoringConditionsStore {
  ...
  async setMonitoringConditions(key: string, token: MonitoringConditionsModel, durationSeconds: number): Promise<void> {
    this.map.set(key, { token: JSON.stringify(token), expiry: new Date(Date.now() + durationSeconds * 1000) })
  }

  async getMonitoringConditions(key: string): Promise<MonitoringConditionsModel | null> {
